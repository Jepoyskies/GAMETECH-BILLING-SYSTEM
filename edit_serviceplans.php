<?php
require 'database.php'; // Load DB connection first

// Redirect if no id provided
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    header('Location: serviceplans.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($plan_name === '' || $speed_up === '' || $speed_down === '' || $price === '' || $validity_days === '') {
        ...
        $sql = "UPDATE service_plans 
                SET plan_name = :plan_name, speed_up = :speed_up, speed_down = :speed_down, price = :price, validity_days = :validity_days, description = :description
                WHERE id = :id";
        ...
        if ($result) {
            ...
            // Refresh $plan with new values, so form stays updated
            $plan = [
                'plan_name'     => $plan_name,
                'speed_up'      => $speed_up,
                'speed_down'    => $speed_down,
...
// Fetch current plan data if not success (so we don't override edited values)
if (!$success) {
    $sql = "SELECT * FROM service_plans WHERE id = :id";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([':id' => $id]);
    $plan = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$plan) {
        header('Location: serviceplans.php?notfound=1');
        exit;
    }
}
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content border-success">
      ...
      <div class="modal-body text-center">
        Service plan was updated successfully.
      </div>
...
<main class="container py-4">
    <h2 class="mb-4 text-center">Edit Service Plan</h2>
