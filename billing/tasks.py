...
def auto_process_sla_rebates():
    ...
    try:
        ...
        open_tickets = DispatchRecord.objects.filter(
            done_at__isnull=True, customer__isnull=False
        )
            ...
            if hours_open >= 24:
                ...
                if owed_days > ticket.sla_rebates_given:
                    ...
                    if customer.expires_at:
                        ...
                        # Dynamic Status Update based on SLA rebate time
                        was_suspended = customer.status in [
                            "suspended",
                            "inactive",
                            "expired",
                        ]
                        ...
                        template_sms = MessageTemplate.objects.filter(
                            type="SMS", name__icontains="SLA"
                        ).first()
                        if not template_sms:
                            template_sms = MessageTemplate.objects.filter(
                                type="SMS"
                        ...
                        if template_sms and customer.phone:
                            sms_msg = (
                                template_sms.body
                                if "SLA" in template_sms.name
                                else f"Hello {customer.full_name}, an SLA Rebate of {days_to_add} days has been added to your account for ticket #{ticket.id}. New expiry: {new_expiry.strftime('%b %d, %Y')}."
                            )
                            for k, v in context.items():
                                sms_msg = sms_msg.replace(k, v)

                            try:
                                semaphore_api = getattr(
                                ...
                                if semaphore_api:
                                    requests.post(
                                        ...
                                        data={
                                            ...
                                            "message": sms_msg,
                                            "sendername": getattr(
                                                settings, "SEMAPHORE_SENDER_NAME", ""
                                            ),
                                        },
                                        timeout=5,
                                    )
                        ...
                        template_email = MessageTemplate.objects.filter(
                            type="EMAIL", name__icontains="SLA"
                        ).first()
                        ...
                        if template_email and customer.email:
                            ...
                            try:
                                ...
                                logger.error(
                                    f"Failed to send SLA Email to {customer.email}: {e}"
                                )
        ...
        for service in services:
            ...
            try:
                ...
                output = subprocess.check_output(
                    command, stderr=subprocess.STDOUT, timeout=5
                ).decode()
