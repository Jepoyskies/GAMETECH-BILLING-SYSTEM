...
def subscription_plans_data_api(request):
    ...
    if status_filter == "active":
        customer_list = [
            ...
            if c.expires_at and c.expires_at > soon and c.status not in ["suspended", "inactive"]
        ]
...
def api_top_clients(request):
    ...
    top_clients_qs = (
        payments.values(
            "customer__full_name", "customer__pppoe_username", "customer__plan__name"
        )
        .annotate(total_paid=Sum("amount"), last_payment=Max("created_at"))
        .order_by("-total_paid")[:50]
    )  # Top 50 clients
...
def api_popular_plans(request):
    ...
    top_plans_qs = (
        customers.values("plan__name").annotate(cnt=Count("id")).order_by("-cnt")[:50]
    )

    data = []
    for p in top_plans_qs:
        data.append({"plan_name": p["plan__name"] or "None", "cnt": p["cnt"]})
