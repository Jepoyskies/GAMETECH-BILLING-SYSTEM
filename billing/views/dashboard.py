...
def dashboard_view(request):
    ...
    if not stats:
        ...
        def get_breakdown(qs):
            ...
            for p in recent:
                recent_list.append(
                    {
                        "username": p.username
                        or (p.customer.pppoe_username if p.customer else "Unknown"),
                        "amount": float(p.amount),
                        "method": p.payment_method,
                        "date": p.created_at,
                    }
                )
            return {"methods": methods, "recent": recent_list}
        ...
        total_last_month = (
            payments.filter(
                created_at__month=last_month_dt.month,
                created_at__year=last_month_dt.year,
        ...
        if total_last_month > 0:
            growth_percent = (
                (float(total_month) - float(total_last_month)) / float(total_last_month)
            ) * 100
        else:
        ...
        collection_rate = (
            (distinct_payers_this_month / total_customers * 100)
        ...
        for i in range(11, -1, -1):
            ...
            m_total = (
                payments.filter(created_at__month=m, created_at__year=y).aggregate(
                    t=Sum("amount")
                )["t"]
                or 0
            )
        ...
        for i in range(6, -1, -1):
            ...
            d_total = (
                payments.filter(created_at__date=d).aggregate(t=Sum("amount"))["t"] or 0
            )
        ...
        stats = {
            ...
            "days_js": days_js,
            "sales_by_day_js": sales_by_day_js,
            "pie_labels_js": pie_labels_js,
            "pie_data_js": pie_data_js,
        }
        # Cache for 5 minutes
    ...
    # Top Paying Clients
    top_clients_qs = (
        payments.values("customer__full_name", "customer__pppoe_username")
        .annotate(total_paid=Sum("amount"), last_payment=Max("created_at"))
    ...
    for c in top_clients_qs:
        if c["customer__full_name"] or c["customer__pppoe_username"]:
            top_clients.append(
                {
                    "username": c["customer__pppoe_username"]
                    or c["customer__full_name"],
    ...
    # Top Service Plans
    top_plans_qs = (
        Customer.objects.values("plan__name")
